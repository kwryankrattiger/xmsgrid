...
//------------------------------------------------------------------------------
void GmMultiPolyIntersectionSorterTerse::Sort(GmMultiPolyIntersectorData& a_data,
    ...
    if (tChange[i + 1] - tChange[i] > 1)
    {
      ...
      if (inNext.size() > 1)
      {
        ...
        for (size_t j = 0; j < inNext.size() && !found; ++j)
        {
          ...
          Turn_enum turn = gmTurn(m_d->m_ixs[edgeBeg].m_pt, m_d->m_ixs[edgeEnd].m_pt, centroid);
          if (turn != TURN_RIGHT)
          {
...
void GmMultiPolyIntersectionSorterTerse::SwapAdjacents()
{
  ...
  if (m_d->m_ixs.size() == 2)
  {
    if ((!m_d->m_polys1.empty() && m_d->m_polys1.find(m_d->m_ixs[0].m_i) == m_d->m_polys1.end()) ||
        (!m_d->m_polys2.empty() && m_d->m_polys2.find(m_d->m_ixs[1].m_i) == m_d->m_polys2.end()))
    {
      Swap(0, 1);
    }
    return;
  ...
  std::vector<Pt3d>& a_pts) const
{
  if (m_d->m_ixs.size() == 2)
  {
    ...
    {
      if (!m_d->m_polys1.empty() && !m_d->m_polys2.empty() && m_d->m_polys1 != m_d->m_polys2)
      {
...
/// \param a_pts: Points.
//------------------------------------------------------------------------------
void GmMultiPolyIntersectionSorterTerse::IntersectionsToPolyIdsAndTValuesFor3OrMore(
  VecInt& polyids,
  VecDbl& tvalues,
  std::vector<Pt3d>& a_pts) const
{
    ...
    if (tChange[i + 1] - tChange[i] > 1)
    {
      ...
      if (i == 0)
      {
        ...
        for (size_t j = 0; j < inNeither.size(); ++j)
        {
          ...
          AddToPolyIdsAndTValues(m_d->m_ixs[inNeither[j]], polyids, tvalues, a_pts);
        }
      }
      else
      {
        ...
        for (size_t j = 0; j < inPrev.size(); ++j)
        {
          if (setIdsPrev.find(m_d->m_ixs[inPrev[j]].m_i) == setIdsPrev.end())
          {
            ...
            AddToPolyIdsAndTValues(m_d->m_ixs[inPrev[j]], polyids, tvalues, a_pts);
          }
        }
        ...
        if (setIds.empty())
        {
          ...
          {
            if (setIdsPrev.find(m_d->m_ixs[inNext[j]].m_i) == setIdsPrev.end())
            {
              setIds.insert(m_d->m_ixs[inNext[j]].m_i);
              AddToPolyIdsAndTValues(m_d->m_ixs[inNext[j]], polyids, tvalues, a_pts);
            }
          }
        }
...
//------------------------------------------------------------------------------
void GmMultiPolyIntersectionSorterTerse::AddToPolyIdsAndTValues(const ix& a_ix,
                                                                VecInt& polyids,
                                                                VecDbl& tvalues,
                                                                std::vector<Pt3d>& a_pts) const
{
...
//------------------------------------------------------------------------------
void GmMultiPolyIntersectionSorterTerse::FindPreviousNextNeither(const VecInt& tChange,
                                                                 const int idx,
                                                                 VecInt* inPrev,
                                                                 VecInt* inNext,
                                                                 VecInt* inNeither) const
{
  ...
  for (int j = tChange[idx]; j < tChange[idx + 1]; ++j)
  {
    ...
    {
      for (int k = tChange[idx + 1]; k < (int)tChange[idx + 2] && !foundNext; ++k)
      {
        if (m_d->m_ixs[j].m_i == m_d->m_ixs[k].m_i)
        {
          foundNext = true;
...
//------------------------------------------------------------------------------
void GmMultiPolyIntersectionSorterTerse::FindWhereTValuesChange(VecInt& tChange) const
{
  tChange.assign(1, 0);
